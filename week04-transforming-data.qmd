...
ggplot(data = penguins_male_female, aes(x = body_mass_g, y = culmen_length_mm, color = species, shape = island)) +
  geom_point() +
  theme_bw() +
  scale_color_viridis_d() +
  facet_wrap(~sex) + 
   labs(title = "Penguins_male_female",
       x = "body_mass_g",
       y = "culmen_length_mm") #stopped here!
```
...
```{r, message=FALSE}
ggplot(data = average_flipper_length, aes(x = species, y = average_flipper_length, fill= species)) + 
  geom_bar(stat = "identity")
  theme_bw() +
  coord_flip() +
