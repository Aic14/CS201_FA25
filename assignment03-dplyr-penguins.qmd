...
my_filter<- penguins %>%
  filter(island %in% c("Dream island","Torgerson islands"),
         sex == "female penguins",
         culmen_length_mm >= 39)
```
...
```{r, message=FALSE, warning=FALSE}
my_case_when <- penguins %>%
  mutate(size = case_when(
    body_mass_g >= 4050 ~ "large",
...
final_boss <- penguins %>%
  ...
  summarise(avg_mass_g =round(mean(body_mass_g, na.rm = TRUE)-2),
            .groups = "drop"
       )
